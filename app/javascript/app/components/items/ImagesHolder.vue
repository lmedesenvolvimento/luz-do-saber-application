    ...
    <div class="ls-images-holder">
        <div class="img-holder" :class="size">
            <div v-for="item in items" :key="item.word_text">
                <div v-if="item.remote_image_url" class="imgs" @click="editWord(item)">
                    <img :src="item.remote_image_url.url" @error="getDefaultImage"/>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        async editWord(item) {
            ...
            const response = await this.$axios.get(`/words.json?q[type_eq]=${word_type}&q[text_eq]=${item.word_text}`)
