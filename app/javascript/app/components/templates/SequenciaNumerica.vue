      ...
      <div class="row">
        <div class="col-md-7">
          <div class="hide-numbers-input-group">
            <label class="label is-top">Números escondidos:</label>
            <span class="input">
              <ls-select-inputs
                :max-items="5"
                @change="onInput"
                :values="inputsNumbers"
              />
            </span>
          </div>
        </div>
...
export default {
  ...
  computed: {
    validate() {
      return (
        this.items.filter(({ type }) => type === 'key').length > 0 &&
        this.distratores > 0
      )
    }
  },
  created() {
    this.inputsNumbers = this.fillUntilValue(12, { text: '' })
  },
  methods: {
    onInput({ data }) {
      ...
      this.items = this.inputsNumbers
        .map(({ text }) => {
          if (!text) return
          if (data.findIndex(item => text === item.text) >= 0) {
